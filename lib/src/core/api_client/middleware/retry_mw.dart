...
class ApiClientRetryMiddleware implements ApiClientMiddleware {
  ...
  ApiClientHandler call(ApiClientHandler innerHandler) => (request, context) async {
    for (var i = 0; i <= maxRetries; i++) {
      try {
        // Clone the request on retries because BaseRequest is finalized after send().
        final effectiveRequest = i == 0 ? request : ApiClientRequest(cloneBaseRequest(request));
        final response = await innerHandler(effectiveRequest, context);
        return response;
      } catch (error) {
